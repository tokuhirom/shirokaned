...
{
  char buf[sizeof(RECV_PACKET)-1];
  int r;
  
  assert((revents & PICOEV_READ) != 0);
  
  r = read(fd, buf, sizeof(buf));
  assert(r == sizeof(RECV_PACKET)-1);
  assert(memcmp(buf, RECV_PACKET, sizeof(RECV_PACKET)-1) == 0);
  ++num_responses;
  ...
  if (num_responses < num_requests) {
    ...
    r = write(wfd, SEND_PACKET, sizeof(SEND_PACKET)-1);
    assert(r == sizeof(SEND_PACKET)-1);
  }
}
...
int main(int argc, char** argv)
{
  ...
  for (i = 0; i < active_connections; ++i) {
    r = write(fds[next_fd_idx], SEND_PACKET, sizeof(SEND_PACKET)-1);
    assert(r == sizeof(SEND_PACKET)-1);
    next_fd_idx = (next_fd_idx + 1) % num_connections;
  }
  /* the main loop */
